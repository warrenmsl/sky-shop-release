...
import { crawlTemuKeyword, extractCategoryMatches } from "./market-crawler/temu-crawler.mjs";
import {
  DATA_DIR,
  RESULTS_FILE,
  SCREENSHOT_DIR,
...
async function processNextTask() {
  ...
  try {
    while (true) {
      ...
      try {
        const result = await crawlTemuKeyword(nextTask);
...
app.get("/api/market/tasks", async (_request, response) => {
  const tasks = await readTasks();
  response.json(tasks.sort((left, right) => right.createdAt.localeCompare(left.createdAt)));
});
...
app.get("/api/market/results", async (request, response) => {
  const platform = request.query.platform ? String(request.query.platform) : undefined;
  const keyword = request.query.keyword ? String(request.query.keyword) : undefined;
  const results = await getFilteredResults({ platform, keyword });
  response.json(results);
});

app.get("/api/market/analysis", async (request, response) => {
  const platform = request.query.platform ? String(request.query.platform) : "temu";
  const keyword = request.query.keyword ? String(request.query.keyword) : undefined;
